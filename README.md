...
1. 「素材を読み込む」で動画・画像・音声を追加します。複数ファイルとドラッグ＆ドロップに対応します。アニメーション GIF は画像素材として読み込み、クリップを伸ばした分だけ先頭から繰り返し再生します（ブラウザが ImageDecoder に対応していない場合は静止画として扱います）。
2. 素材の画像または ＋ を押すか、タイムラインへドラッグします。オーディオパネルからは動画の音声だけも追加できます。
3. クリップをドラッグして移動、左右の端でトリミングします。クリップを選択して再生位置で `⌘ / Ctrl + S` を押すと分割します。`⌘ / Ctrl + Shift + S` で保存します。
4. テキスト・図形は下部のツールから追加します。追加した素材は種類に関係なく任意のトラックへ配置できます。
5. インスペクターで位置、倍率、回転、クロップ、不透明度、文字装飾、音量、フェードを編集します。タブレットではクリップの長押しメニューから「詳細を編集」を選びます。
...
- クリップを約0.5秒長押しすると、分割・削除・複製・移動・詳細編集のコンパクトなメニューが押した位置の近くに開きます。メニュー外をタップすると閉じます。分割は常に再生バーの位置で行います。
- クリップ上をスワイプするとスクロールします。移動する場合はメニューで「移動」を選んでからドラッグしてください。映像・音声・画像・テキストなど、どのクリップも任意のトラックへ移動できます。
- タップでクリップを選択すると、左右にトリム用の黄色いハンドルが表示されます。
- 目盛りと空きレーンは左クリック／左ドラッグで再生位置を移動できます。クリップの操作メニューは右クリック、キーボードは Shift+F10 に対応します。スナップとズームはタイムライン右上で調整できます。
- ロック中の編集と、先頭位置での分割は無効です。削除は上部の「元に戻す」で取り消せます。

## パネルの配置

パネル間の紫色の境界線をドラッグすると幅・高さを変更できます。境界線はキーボードでフォーカスして矢印キーでも調整できます。各パネル上部の × で閉じ、ヘッダーの「表示」から再び開けます。「配置をリセット」で比率と表示状態を初期状態に戻せます。素材パネルは下部の素材ツールからも開けます。編集設定は「表示」またはクリップの長押しメニューから開けます。配置はこのブラウザに保存され、編集データや履歴には影響しません。プレビューを閉じると再生は停止します。狭い画面では上下方向に分割します。
...
- 動画を書き出した後は、対応ブラウザでは保存先を選択できます。未対応ブラウザでは従来どおりダウンロードフォルダーへ保存します。
...
同期はアプリが開いていて、オンラインかつ認証が有効な間に行います。失敗したキューと再開可能アップロードのセッションを IndexedDB に残します。同期競合を検出した場合は元のデータを上書きせず別案を追加します。project.jsonのETag取得とIf-Match付き更新はCloudflare Worker経由で行い、検証情報を取得できない場合は既存ファイルを書き換えません。同じ共有プロジェクトを閲覧者が編集した場合は、元ファイルではなく閲覧者自身のDriveへ派生版として保存します。
